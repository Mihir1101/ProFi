...
def it_was_zero_so_we_made_it_one(val1,val2,val3,val4):
    if(abs(val1 - val2) == 0 and (val3 - val4) == 0):
        return 1
    elif(abs(val3 - val4) == 0):
        return 1
    else:
        return abs((val1 - val2) / (val3 - val4))
...
with open('PEPE_ALL_graph_coinmarketcap.csv', 'r') as csv_file:
...
markets_for_pepe = 103
github_commits_for_pepe = 735

algo_array = np.zeros(np.size(open_array))

for i in range(len(algo_array) - 1):
    algo_array[i] = (2 * normalize_between_zero_and_one(it_was_zero_so_we_made_it_one(high_array[i],low_array[i],close_array[i],open_array[i]))) + (2 * (1 - (abs(marketCap_array[i] - marketCap_array[i+1])) / 100000000000)) + (2 * (1 - (abs(volume_array[i] - volume_array[i+1])) / 100000000000)) + (1 * ((markets_for_pepe) / 10000)) + (2 * ((github_commits_for_pepe) / 100000))+ buddy(close_array[i],open_array[i])
