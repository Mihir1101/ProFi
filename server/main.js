...
function Listen_For_Events(contract) {
    const eventSubscription = contract.events["twitter_followers_changed", "markets_changed", "github_commits_changed"]({
...
app.get("/:token", async (req, res) => {
    if (req.params.token == "favicon.ico") { } else {
        ...
        res.status(200).render("index", { token: req.params.token, twitter_followers: Twitter_Followers, Github_Commits: Github_Commits, Markets: Markets });
    }
});
